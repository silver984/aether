...
private:
	animation_repository();
	void clear_cache_();
	[[nodiscard]] std::shared_ptr<animation_map> try_fetch_from_cache_(std::filesystem::path const& file) const;
	[[nodiscard]] std::shared_ptr<animation_map> xml_parse_(std::filesystem::path const& file);
	...
	[[nodiscard]] std::shared_ptr<animation_map> xml_adobe_animate_parse_(tinyxml2::XMLDocument const& document);
	[[nodiscard]] std::shared_ptr<animation_map> xml_texture_packer_parse_(tinyxml2::XMLDocument const& document);
